...
{
	if ((SC_ServiceHandle = OpenService(SC_ManagerHandle, /*DRIVER_NAME*/L"Lab5", SERVICE_ALL_ACCESS)) ||
	...
	{
		wcout << L"Error find Service Lab5" << endl;
	}
}
...
SC_HANDLE createService()
{
	...
	wcscpy(lastSlach ? &lastSlach[1] : fullFileName, /*DRIVER_NAME*/ L"Lab5.sys");

	isCreateService = true;
	return CreateService(SC_ManagerHandle, /*DRIVER_NAME, DRIVER_NAME*/L"Lab5", L"Lab5", SERVICE_ALL_ACCESS, SERVICE_KERNEL_DRIVER, SERVICE_DEMAND_START,
...
void syncWithDriver()
{
	controlDriver = CreateFile(L"\\\\.\\"/*DRIVER_NAME*/L"Lab5", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	if (controlDriver != INVALID_HANDLE_VALUE)
	{
		...
		wcout << L"Error CreateFile Lab5"/*DRIVER_NAME*/ << endl;
}
...
{
	HANDLE hEvent = OpenEvent(SYNCHRONIZE, FALSE, L"Global\\CreateProcEvent"/*SYNC_CREATE_PROC_EVENT*/);
	do
	{
...
{
	HANDLE hEvent = OpenEvent(SYNCHRONIZE, FALSE, L"Global\\CloseProcEvent");
	do
	{
		DWORD status = WaitForSingleObject(hEvent, 1000);
